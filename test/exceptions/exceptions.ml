...
let expand_into_extension_node ~ctxt =
  ...
  let extension_node = Location.Error.(make ~loc "An error message in an extension node" ~sub:[]
                                        |> to_extension) in
     Ast_builder.Default.pexp_extension ~loc extension_node

let expand_raise_exception ~ctxt:_ =
...
let () = Driver.register_transformation ~rules:[ rule1 ] "gen_ext_node"
let () = Driver.register_transformation ~rules:[ rule2 ] "gen_raise_exc"
let () = Driver.register_transformation ~rules:[ rule3 ] "gen_raise_located_error"
