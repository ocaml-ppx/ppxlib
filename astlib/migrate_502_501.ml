...
and copy_value_binding :
 ...
 fun {
     ...
     } ->
  {
    ...
    Ast_501.Parsetree.pvb_constraint = Option.map copy_poly_constraint pvb_constraint;
...
and copy_poly_constraint : Ast_502.Parsetree.poly_constraint -> Ast_501.Parsetree.poly_constraint
    =
 fun {
        Ast_502.Parsetree.locally_abstract_univars;
        Ast_502.Parsetree.typ;
     } ->
  {
    Ast_501.Parsetree.locally_abstract_univars = List.map (copy_loc (fun x -> x)) locally_abstract_univars;
...
and copy_module_expr_desc :
  ...
  | Ast_502.Parsetree.Pmod_apply_unit x0 ->
      ...
      in
      Ast_501.Parsetree.Pmod_apply (copy_module_expr x0, empty_struct)
