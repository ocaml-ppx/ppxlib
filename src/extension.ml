...
module For_context = struct
  ...
  let exn_to_extension exn str =
    ...
    | None ->
        Location.Error.make ~loc:str.loc
          ("(PPX " ^ str.txt ^ ":) " ^ Printexc.to_string exn ^ "\n"
         ^ "(Ppxlib:) " ^ str.txt
         ^ " has violated the PPX norm on how to report errors. Precise \
            location of error cannot be given.")
          ~sub:[]
    | Some error ->
  ...
  let convert ?(embed_errors = false) ts ~ctxt ((str, _) as ext) context x =
    ...
    | Some ({ payload = M.Payload_parser (pattern, f); _ }, arg) -> (
        ...
        | exn -> raise @@ ContextFreeException (str.txt, exn))
