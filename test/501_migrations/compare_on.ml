let run () =
  let example_fn, ppx =
    ...
    if not (Array.length args = 3) then
      failwith "expected exactly two args"
    else Array.get args 1, Array.get args 2
  in
  let direct = "without_migrations" in
  let migrations = "with_migrations" in
  let direct_ec = Sys.command ("ocamlopt -dparsetree " ^ example_fn ^ " 2> " ^ direct) in
  if direct_ec > 0 then
    (print_endline "compile error even without migrations";
    let _ = Sys.command ("cat " ^ direct) in
    ())
  else
  let migrations_ec = Sys.command ("ocamlopt -dparsetree -ppx '" ^ ppx ^ " -as-ppx' " ^ example_fn ^ " 2> " ^ migrations) in
