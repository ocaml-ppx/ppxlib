...
and pattern_desc = Parsetree.pattern_desc =
  ...
  | Ppat_tuple of pattern list
      (** Patterns [(P1, ..., Pn)].
          ...
          Invariant: [n >= 2] *)
  | Ppat_construct of longident loc * (string loc list * pattern) option
  ...
  | Ppat_record of (longident loc * pattern) list * closed_flag
...
and expression_desc = Parsetree.expression_desc =
  ...
  | Pexp_record of (longident loc * expression) list * expression option
  ...
  | Pparam_val of arg_label * expression option * pattern
      (** [Pparam_val (lbl, exp0, P)] represents the parameter:
          ...
          - [?l:P] when [lbl] is {{!Asttypes.arg_label.Optional} [Optional l]}
            and [exp0] is [None]
          - [?l:(P = E0)] when [lbl] is {{!Asttypes.arg_label.Optional}
            [Optional l]} and [exp0] is [Some E0]

          Note: If [E0] is provided, only {{!Asttypes.arg_label.Optional}
          [Optional]} is allowed. *)
...
class virtual map =
  object (self)
    ...
    method module_type_desc : module_type_desc -> module_type_desc=
      fun x ->
        match x with
        | Pmty_ident a -> let a = self#loc self#longident a in Pmty_ident a
        ...
        | Pmty_alias a -> let a = self#loc self#longident a in Pmty_alias a
    ...
    method module_expr_desc : module_expr_desc -> module_expr_desc=
      fun x ->
        match x with
        | Pmod_ident a -> let a = self#loc self#longident a in Pmod_ident a
  ...
  object (self)
    ...
    method core_type_desc : core_type_desc -> unit=
      fun x ->
        ...
        | Ptyp_class (a, b) ->
            (self#loc self#longident a; self#list self#core_type b)
        | Ptyp_alias (a, b) -> (self#core_type a; self#loc self#string b)
        | Ptyp_variant (a, b, c) ->
        ...
        | Ptyp_open (a, b) -> (self#loc self#longident a; self#core_type b)
        | Ptyp_extension a -> self#extension a
    method package_type : package_type -> unit=
      fun (a, b) ->
        self#loc self#longident a;
        self#list (fun (a, b) -> self#loc self#longident a; self#core_type b)
          b
    ...
    method pattern_desc : pattern_desc -> unit=
      fun x ->
        ...
        | Ppat_record (a, b) ->
            (self#list
               (fun (a, b) -> self#loc self#longident a; self#pattern b) a;
             self#closed_flag b)
    ...
    method expression_desc : expression_desc -> unit=
      fun x ->
        ...
        | Pexp_record (a, b) ->
            (self#list
               (fun (a, b) -> self#loc self#longident a; self#expression b) a;
             self#option self#expression b)
        | Pexp_field (a, b) -> (self#expression a; self#loc self#longident b)
        | Pexp_setfield (a, b, c) ->
            (self#expression a; self#loc self#longident b; self#expression c)
    ...
    method function_body : function_body -> unit=
      fun x ->
        ...
        | Pfunction_cases (a, b, c) ->
            (self#cases a; self#location b; self#attributes c)
    ...
    method with_constraint : with_constraint -> unit=
      fun x ->
        ...
        | Pwith_modtype (a, b) ->
            (self#loc self#longident a; self#module_type b)
        | Pwith_modtypesubst (a, b) ->
            (self#loc self#longident a; self#module_type b)
        | Pwith_typesubst (a, b) ->
            (self#loc self#longident a; self#type_declaration b)
      ...
      fun ctx ->
        fun x ->
          ...
          | Ptyp_alias (a, b) ->
              ...
              let b = self#loc self#string ctx b in Ptyp_alias (a, b)
          | Ptyp_variant (a, b, c) ->
          ...
          | Ptyp_open (a, b) ->
              let a = self#loc self#longident ctx a in
              let b = self#core_type ctx b in Ptyp_open (a, b)
          | Ptyp_extension a ->
    ...
    method expression_desc : 'ctx -> expression_desc -> expression_desc=
      fun ctx ->
        fun x ->
          ...
          | Pexp_new a -> let a = self#loc self#longident ctx a in Pexp_new a
          | Pexp_setinstvar (a, b) ->
              let a = self#loc self#label ctx a in
    ...
    method function_param_desc :
      ...
      fun ctx ->
        fun x ->
          ...
          | Pparam_newtype a ->
              let a = self#loc self#string ctx a in Pparam_newtype a
    ...
    method with_constraint : 'ctx -> with_constraint -> with_constraint=
      fun ctx ->
        fun x ->
          ...
          | Pwith_modsubst (a, b) ->
              let a = self#loc self#longident ctx a in
              let b = self#loc self#longident ctx b in Pwith_modsubst (a, b)
  ...
  object (self)
    ...
    method core_type_desc : core_type_desc -> 'res=
      fun x ->
        ...
        | Ptyp_alias (a, b) ->
            let a = self#core_type a in
            let b = self#loc self#string b in self#constr "Ptyp_alias" [a; b]
        | Ptyp_variant (a, b, c) ->
            let a = self#list self#row_field a in
            let b = self#closed_flag b in
            let c = self#option (self#list self#label) c in
        ...
        | Ptyp_open (a, b) ->
            let a = self#loc self#longident a in
            let b = self#core_type b in self#constr "Ptyp_open" [a; b]
        | Ptyp_extension a ->
    ...
    method pattern_desc : pattern_desc -> 'res=
      fun x ->
        ...
        | Ppat_type a ->
            let a = self#loc self#longident a in self#constr "Ppat_type" [a]
        | Ppat_lazy a ->
    ...
    method expression_desc : expression_desc -> 'res=
      fun x ->
        match x with
        | Pexp_ident a ->
            let a = self#loc self#longident a in self#constr "Pexp_ident" [a]
        | Pexp_constant a ->
        ...
        | Pexp_new a ->
            let a = self#loc self#longident a in self#constr "Pexp_new" [a]
        | Pexp_setinstvar (a, b) ->
            let a = self#loc self#label a in
    ...
    method module_type_desc : module_type_desc -> 'res=
      fun x ->
        match x with
        | Pmty_ident a ->
            let a = self#loc self#longident a in self#constr "Pmty_ident" [a]
        | Pmty_signature a ->
        ...
        | Pmty_alias a ->
            let a = self#loc self#longident a in self#constr "Pmty_alias" [a]
    ...
    method module_expr_desc : module_expr_desc -> 'res=
      fun x ->
        match x with
        | Pmod_ident a ->
            let a = self#loc self#longident a in self#constr "Pmod_ident" [a]
        | Pmod_structure a ->
